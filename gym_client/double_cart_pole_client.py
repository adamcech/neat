...
class DoubleCartPoleClient(GymClient):

    def get_environment(self) -> gym.Env:
        return DoubleCartPoleEnv()
    ...
    def _get_ann_action(self, ann: Ann, observation: np.ndarray) -> int:
        obs = np.append([observation[i] for i in range(len(observation))], [1 for _ in range(self.get_bias_size())])
        output = ann.calculate(obs)
