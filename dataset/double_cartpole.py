...
class DoubleCartPole(gym.Env):
    ...
    def __init__(self):
        self.seed_int = np.random.randint(0, 1000000)
    ...
    def step(self, action):
        ...
        done = x < -self.x_threshold or x > self.x_threshold or self.counter > 1000 or theta > 90 * 2 * np.pi / 360 or theta < -90 * 2 * np.pi / 360
        done = bool(done)

        cost = 10 * normalize_angle(theta) + 10 * normalize_angle(phi)
        reward = cost

        return self.convert_state(), 1.0, done, {}

    def convert_state(self):
        return [float(v) for v in list(self.state)]

    def seed(self, seed=None):
        self.seed_int = np.random.randint(0, 1000000) if seed is None else seed
        self.reset()
        return [self.seed_int]
